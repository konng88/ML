...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        word_ids = [[[self.char2id['{']]+[self.char2id[char] for char in word]+[self.char2id['}']] for word in sent] for sent in sents]
        return word_ids
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_var = pad_sents_char(self.words2charindices(sents),self.char2id['<pad>'])
        return torch.tensor(sents_var,dtype=torch.long,device=device).permute((1,0,2))
