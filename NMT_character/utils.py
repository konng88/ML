    ...
    for sent in sents:
        ...
        for word in sent:
            if len(word) < max_word_length:
                pad_word = word + [char_pad_token]*(max_word_length-len(word))
            else:
                pad_word = word[:max_word_length]
            sent_padded.append(pad_word)
        if len(sent) < max_sent_length:
            for _ in range(max_sent_length-len(sent)):
                sent_padded.append([char_pad_token]*max_word_length)
...
def pad_sents(sents, pad_token):
    ...
    maxL = max(len(line) for line in sents)
    for i in range(len(sents)):
        for j in range(maxL-len(sents[i])):
