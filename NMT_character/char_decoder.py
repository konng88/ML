...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        o_t, (h_t, c_t) = self.charDecoder(X_embedded, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        p_t = nn.functional.softmax(s_t,dim=0)
        ce = nn.CrossEntropyLoss()
        loss = ce(p_t.permute((1,2,0)),char_sequence[1:,:].permute((1,0)))
        return loss
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        for i in range(max_length-1):
            X_emb = self.decoderCharEmb(current_char)
            o_t , (h_t, c_t) = self.charDecoder(X_emb, (h_t, c_t))
            s_t = self.char_output_projection(o_t)
            p_t = nn.functional.softmax(s_t,dim=-1)
            output_words.append(torch.argmax(p_t,dim=2))
        output_words = torch.squeeze(torch.stack(output_words,dim=0),dim=1)
        output_words = output_words.t().tolist()
        for charids in output_words:
            word = ''
            for charid in charids:
                if self.target_vocab.id2char[charid] == self.target_vocab.end_of_word:
                    break
                word += self.target_vocab.id2char[charid]
            decodedWords += [word]
        return decodedWords
        ### END YOUR CODE
