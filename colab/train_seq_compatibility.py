...
if __name__=='__main__':
    ...
    model = Model(inputs=[inp,masks], outputs=x)

    def scheduler(epoch):
        return 0.005 * (0.4**(epoch//5))

    callback = tf.keras.callbacks.LearningRateScheduler(scheduler)
    saver = tf.keras.callbacks.ModelCheckpoint(filepath='model',save_best_only=True,verbose=1)
